...
def main(verbose=False):
    ...
    # Standard deviation of variance and covariance terms in int_cov, & int_dcov_dcorr_reg
    std_var = np.zeros(common_shape, dtype=float)
    std_cov = np.zeros(common_shape, dtype=float)
    std_var_est = np.zeros(common_shape, dtype=float)
    std_cov_est = np.zeros(common_shape, dtype=float)
    std_var_nonlinear = np.zeros(common_shape, dtype=float)
    std_cov_nonlinear = np.zeros(common_shape, dtype=float)
    ...
    # linear shrinkage on covariance matrix
    selections_shrink = np.zeros(common_shape + (len(shrinking_factors), L), dtype=float)
    spearmanr_shrink = np.zeros(common_shape + (len(shrinking_factors), ), dtype=float)
    error_shrink = np.zeros(common_shape + (len(shrinking_factors), ), dtype=float)
    # combining multiple replicates
    selections_shrink_combine = np.zeros(combine_shape + (len(shrinking_factors), L), dtype=float)
    spearmanr_shrink_combine = np.zeros(combine_shape + (len(shrinking_factors), ), dtype=float)
    error_shrink_combine = np.zeros(combine_shape + (len(shrinking_factors), ), dtype=float)
    ...
    # non-linear shrinkage on correlation matrix and additional shrinking on covariances (off-diagonal terms)
    selections_dcorr_shrink = np.zeros(common_shape + (len(shrinking_factors), L), dtype=float)
    spearmanr_dcorr_shrink = np.zeros(common_shape + (len(shrinking_factors), ), dtype=float)
    error_dcorr_shrink = np.zeros(common_shape + (len(shrinking_factors), ), dtype=float)
    # combining multiple replicates
    selections_dcorr_shrink_combine = np.zeros(combine_shape + (len(shrinking_factors), L), dtype=float)
    spearmanr_dcorr_shrink_combine = np.zeros(combine_shape + (len(shrinking_factors), ), dtype=float)
    error_dcorr_shrink_combine = np.zeros(combine_shape + (len(shrinking_factors), ), dtype=float)
        ...
        for n in range(num_trials):
            ...
            for p in range(len(sample)):
                for q in range(len(record)):
                    ...
                    dic = np.load(arg_list.i + f'_s={s}_n={n}_sample={sample[p]}_record={record[q]}.npz', allow_pickle=True)
                    mu = dic['mu']
                    ...
                    std_var[s, n, p, q] = np.std(EC.get_diagonal_terms(int_cov[s, n, p, q]))
                    std_cov[s, n, p, q] = np.std(EC.get_off_diagonal_terms(int_cov[s, n, p, q]))

                    int_corr[s, n, p, q] = EC.computeCorrelation(int_cov[s, n, p, q])
                    ...
                    int_dcov[s, n, p, q] = EC.estimateIntegratedCovarianceMatrix(traj, times, window, population=N)
                    int_dcorr[s, n, p, q] = EC.computeCorrelation(int_dcov[s, n, p, q])

                    std_var_est[s, n, p, q] = np.std(EC.get_diagonal_terms(int_dcov[s, n, p, q]))
                    std_cov_est[s, n, p, q] = np.std(EC.get_off_diagonal_terms(int_dcov[s, n, p, q]))

                    # estimated integrated covariance (uncalibrated)
                    int_dcov_uncalibrated[s, n, p, q]= EC.estimateIntegratedCovarianceMatrix(traj, times, window, population=N, calibrate=False)
                    int_dcorr_uncalibrated_tmp = EC.computeCorrelation(int_dcov_uncalibrated[s, n, p, q])

                    # performance on estimating the covariance with calibrated or uncalibrated cov
                    spearmanr_cov_calibrated[s, n, p, q], error_cov_calibrated[s, n, p, q] = EC.computeMetricsForCovarianceEstimation(int_dcov[s, n, p, q], int_cov[s, n, p, q])
                    spearmanr_cov_uncalibrated[s, n, p, q], error_cov_uncalibrated[s, n, p, q] = EC.computeMetricsForCovarianceEstimation(int_dcov_uncalibrated[s, n, p, q], int_cov[s, n, p, q])
                    ...
                    for i in range(num_basic):
                        ...
                        (spearmanr_basic[s, n, p, q, i],
                         error_basic[s, n, p, q, i]) = EC.computePerformance(selections_basic[s, n, p, q, i], selections[s])
                    ...
                    for r in range(len(linear)):
                        ...
                        (spearmanr_linear[s, n, p, q, r],
                         error_linear[s, n, p, q, r]) = EC.computePerformance(selections_linear[s, n, p, q, r], selections[s])
                    ...
                    for r in range(len(linear)):
                        ...
                        spearmanr_cov_calibrated_linear[s, n, p, q, r], error_cov_calibrated_linear[s, n, p, q, r] = EC.computeMetricsForCovarianceEstimation(int_dcov_calibrated_linear_tmp, int_cov[s, n, p, q])
                        int_dcov_uncalibrated_linear_tmp = int_dcov_uncalibrated[s, n, p, q] + record[q] * linear[r] * IdentityMatrix
                        spearmanr_cov_uncalibrated_linear[s, n, p, q, r], error_cov_uncalibrated_linear[s, n, p, q, r] = EC.computeMetricsForCovarianceEstimation(int_dcov_uncalibrated_linear_tmp, int_cov[s, n, p, q])

                    # Shrinking on covariances (off-diagonal terms)
                    for i, factor in enumerate(shrinking_factors):
                        cov_tmp = EC.shrinkOffDiagonalTerms(int_dcov[s, n, p, q], factor)
                        selections_shrink[s, n, p, q, i] = MPL.inferSelection(cov_tmp, D[s, n, p, q], 1 * IdentityMatrix)
                        (spearmanr_shrink[s, n, p, q, i],
                         error_shrink[s, n, p, q, i]) = EC.computePerformance(selections_shrink[s, n, p, q, i], selections[s])

                    # Non-linear shrinkage on integrated correlation matrix
                    multiplier_corr_to_cov = EC.computeCorrToCovMultiplier(int_dcov[s, n, p, q])
                    for l in range(len(loss)):
                        for g in range(len(gamma)):
                            int_dcorr_reg[s, n, p, q, l, g] = EC.shrinkCorrelation(int_dcorr[s, n, p, q], l, gamma[g])
                            ...
                            (spearmanr_dcorr[s, n, p, q, l, g],
                             error_dcorr[s, n, p, q, l, g]) = EC.computePerformance(selections_dcorr[s, n, p, q, l, g], selections[s])

                    std_var_nonlinear[s, n, p, q] = np.std(EC.get_diagonal_terms(int_dcov_dcorr_reg[s, n, p, q, loss_selected, gamma_selected]))
                    std_cov_nonlinear[s, n, p, q] = np.std(EC.get_off_diagonal_terms(int_dcov_dcorr_reg[s, n, p, q, loss_selected, gamma_selected]))

                    # Performance of estimating covariance with calibrated and nonlinearly regularized cov
                    spearmanr_cov_calibrated_nonlinear[s, n, p, q], error_cov_calibrated_nonlinear[s, n, p, q] = EC.computeMetricsForCovarianceEstimation(int_dcov_dcorr_reg[s, n, p, q, loss_selected, gamma_selected], int_cov[s, n, p, q])

                    # Performance of estimating covariance with uncalibrated and nonlinearly regularized cov
                    multiplier_corr_to_cov_uncalibrated = EC.computeCorrToCovMultiplier(int_dcov_uncalibrated[s, n, p, q])
                    int_dcorr_uncalibrated_reg_tmp = EC.shrinkCorrelation(int_dcorr_uncalibrated_tmp, loss_selected, gamma[gamma_selected])
                    ...
                    spearmanr_cov_uncalibrated_nonlinear[s, n, p, q], error_cov_uncalibrated_nonlinear[s, n, p, q] = EC.computeMetricsForCovarianceEstimation(int_dcov_uncalibrated_dcorr_reg[s, n, p, q], int_cov[s, n, p, q])

                    # Non-linear shrinkage and additional shrinking on covariances (off-diagonal terms)
                    for i, factor in enumerate(shrinking_factors):
                        cov_tmp = EC.shrinkOffDiagonalTerms(int_dcov_dcorr_reg[s, n, p, q, loss_selected, gamma_selected], factor)
                        selections_dcorr_shrink[s, n, p, q, i] = MPL.inferSelection(cov_tmp, D[s, n, p, q], 1 * IdentityMatrix)
                        (spearmanr_dcorr_shrink[s, n, p, q, i],
                         error_dcorr_shrink[s, n, p, q, i]) = EC.computePerformance(selections_dcorr_shrink[s, n, p, q, i], selections[s])
        ...
        for n in range(num_trials):
            for p in range(len(sample)):
                for q in range(len(record)):
                    ...
                    int_dcorr_combine = EC.computeCorrelation(int_dcov_combine)
                    ...
                    for i in range(num_basic):
                        ...
                        (spearmanr_basic_combine[s, n, p, q, i],
                         error_basic_combine[s, n, p, q, i]) = EC.computePerformance(selections_basic_combine[s, n, p, q, i], selections[s])
                    ...
                    for r in range(len(linear)):
                        ...
                        (spearmanr_linear_combine[s, n, p, q, r],
                         error_linear_combine[s, n, p, q, r]) = EC.computePerformance(selections_linear_combine[s, n, p, q, r], selections[s])

                    # Shrinking on covariances (off-diagonal terms)
                    for i, factor in enumerate(shrinking_factors):
                        cov_tmp = EC.shrinkOffDiagonalTerms(int_dcov_combine, factor)
                        selections_shrink_combine[s, n, p, q, i] = MPL.inferSelection(cov_tmp, D_combine, 1 * IdentityMatrix)
                        (spearmanr_shrink_combine[s, n, p, q, i],
                         error_shrink_combine[s, n, p, q, i]) = EC.computePerformance(selections_shrink_combine[s, n, p, q, i], selections[s])

                    # Non-linear shrinkage on integrated correlation matrix
                    multiplier_corr_to_cov_combine = EC.computeCorrToCovMultiplier(int_dcov_combine)
                    for l in range(len(loss)):
                        for g in range(len(gamma)):
                            ...
                            int_dcorr_reg_combine = EC.shrinkCorrelation(int_dcorr_combine, l, gamma[g])
                            ...
                            (spearmanr_dcorr_combine[s, n, p, q, l, g],
                             error_dcorr_combine[s, n, p, q, l, g]) = EC.computePerformance(selections_dcorr_combine[s, n, p, q, l, g], selections[s])

                    int_dcorr_reg_combine = EC.shrinkCorrelation(int_dcorr_combine, loss_selected, gamma[gamma_selected])
                    # Non-linear shrinkage and additional shrinking on covariances (off-diagonal terms)
                    for i, factor in enumerate(shrinking_factors):
                        cov_tmp = EC.shrinkOffDiagonalTerms(int_dcorr_reg_combine, factor)
                        selections_dcorr_shrink_combine[s, n, p, q, i] = MPL.inferSelection(cov_tmp, D_combine, 1 * IdentityMatrix)
                        (spearmanr_dcorr_shrink_combine[s, n, p, q, i],
                         error_dcorr_shrink_combine[s, n, p, q, i]) = EC.computePerformance(selections_dcorr_shrink_combine[s, n, p, q, i], selections[s])
    ...
    selections_dic   = {
                        ...
                        'selections_shrink': selections_shrink, 'selections_shrink_combine': selections_shrink_combine,
                        'selections_dcorr': selections_dcorr, 'selections_dcorr_combine': selections_dcorr_combine,
                        'selections_dcorr_shrink': selections_dcorr_shrink, 'selections_dcorr_shrink_combine': selections_dcorr_shrink_combine}
    performances_dic = {
                        'std_var': std_var, 'std_cov': std_cov,
                        'std_var_est': std_var_est, 'std_cov_est': std_cov_est,
                        'std_var_nonlinear': std_var_nonlinear, 'std_cov_nonlinear': std_cov_nonlinear,
                        ...
                        'spearmanr_shrink': spearmanr_shrink, 'spearmanr_shrink_combine': spearmanr_shrink_combine,
                        'spearmanr_dcorr': spearmanr_dcorr, 'spearmanr_dcorr_combine': spearmanr_dcorr_combine,
                        'spearmanr_dcorr_shrink': spearmanr_dcorr_shrink, 'spearmanr_dcorr_shrink_combine': spearmanr_dcorr_shrink_combine,
                        ...
                        'error_shrink': error_shrink, 'error_shrink_combine': error_shrink_combine,
                        'error_dcorr': error_dcorr, 'error_dcorr_combine': error_dcorr_combine,
                        'error_dcorr_shrink': error_dcorr_shrink, 'error_dcorr_shrink_combine': error_dcorr_shrink_combine}
...
#     spearmanr_all, error_all = EC.computePerformance(est_all, true_all)
#     spearmanr_off_diagonal, error_off_diagonal = EC.computePerformance(est_all[L:], true_all[L:])
#     spearmanr_diagonal, error_diagonal = EC.computePerformance(est_all[:L], true_all[:L])
#
